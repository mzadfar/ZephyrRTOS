...
MEMORY
    ...
    RAM (wx) : ORIGIN = 0x20000000, LENGTH = (192 * 1K)
    CCM : ORIGIN = (268435456), LENGTH = (65536)
 ...
 *(".kernel_noinit.*")
    ...
    __kernel_ram_end = 0x20000000 + (192 * 1K);
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    CCM (NOLOAD) : { __CCM_start = .; KEEP(*(CCM)) KEEP(*(CCM.*)) __CCM_end = .; } > CCM __CCM_size = __CCM_end - __CCM_start; __CCM_load_start = LOADADDR(CCM);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
